...
// let (변수 선언 문법) : 자바의 변수형태와 동일, 중복 선언 불가, 재할당 가능, ES6에서 추가
// const (상수 선언 문법) : 재할당 불가, ES6에서 추가
...
// 문자열(문자와 문자열을 동일하게 사용함)
// let str = '문자열' + " 연결 연산자" + ` 이것도 사용 가능`;
// console.log(str);
...
// let num1 = 10.1;
// console.log(typeof(num1));  // typeof()는 데이터 타입을 확인하는 함수
...
// undefined : 변수는 선언이 되었지만 아직 값이 정해지지 않았음을 의미 (개발자가 대입하는 자료형이 아님)
let empty;
console.log(empty);
// null : 변수나 상수를 선언하고 의도적으로 선언한 공간을 비워 둘 때 사용
let empty2 = null;
console.log(empty2);
// undefined 와 null을 묶어서 특수 자료형이라고도 한다.

// 객체(object) : 자바 스크립트의 핵심 자료형. 
// 기본 자료형을 제외하고 자바스크립트의 대부분의 자료 구조는 객체로 이루어져 있음
...
// 객체 리터럴은 키와 값의 한 쌍으로 구성 (자바의 map과 유사), 중괄호를 이용해서 생성
let score = {
    kor: 80,
    eng: 90,
    mat: 100,
